...
const Calculator = {
    ...
    First_Operand: null,
    //THis checks wether or not the second opreand has been inputted by the user.
...
function Input_Decimal (dot) {
    ...
    //cuase bugs in the operation.
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot)) {
        //We are saying that if the Deisplay_Value does not contain a decimal point
        //we want to add a decimal point.
        Calculator.Display_Value += dot;
    }
}

//This section hadles operators
...
keys.addEventListener('click', (event) =>  {
    //The target variable is an object represents the element
    //that was clicked.
    const { target } = event;
